...
main_folder = 'ReSizedDataset/'  # Adjust based on your setup

# Adjusting the function to also return image paths
def load_and_advanced_preprocess_images(main_folder, target_size=(160, 160), augmentation=True, max_images_per_folder=500):
    ...
    for clone_label in clone_labels:
        ...
        for filename in tqdm(os.listdir(subfolder_path), desc=f"Processing images from {clone_label}"):
            ...
            valid_image_paths.append(img_path)  # Add valid image path

            if augmentation:
                augmented_img = apply_advanced_data_augmentation(img)
                images.append(augmented_img)
                labels.append(label_map[clone_label])
                valid_image_paths.append(img_path)  # Reuse the same path for the augmented image

            loaded_images += 1

    return np.array(images), np.array(labels), valid_image_paths  # Return the list of valid image paths as well

# Adjust your main code to capture the returned image paths
images, labels, image_paths = load_and_advanced_preprocess_images(main_folder, augmentation=True)

# Then, create the DataFrame using these valid image paths
data_dict = {
    "Image_Path": image_paths,  # Use the list of valid image paths
    "Label": labels
}
...
# main_folder = 'ReSizedDataset/'  # Adjust based on your setup
# images, labels = load_and_advanced_preprocess_images(main_folder, augmentation=True)
...
# data_dict = {
#     "Image_Path": [os.path.join(main_folder, subfolder, f) for subfolder in os.listdir(main_folder) for f in os.listdir(os.path.join(main_folder, subfolder))],
#     "Label": y.tolist()
# }
df = pd.DataFrame(data_dict)
df.to_csv('preprocessed-data/image_data.csv', index=False)
...
# X_train_rgb = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in X_train]
# X_test_rgb = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in X_test]
# Before converting BGR to RGB, ensure images are in the correct depth (8-bit unsigned integers)
X_train_rgb = [cv2.cvtColor((img * 255).astype(np.uint8), cv2.COLOR_BGR2RGB) if img.dtype != np.uint8 else cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in X_train]
X_test_rgb = [cv2.cvtColor((img * 255).astype(np.uint8), cv2.COLOR_BGR2RGB) if img.dtype != np.uint8 else cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in X_test]
